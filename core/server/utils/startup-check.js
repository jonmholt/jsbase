var packages = require('../../../package.json'),
    ...
    configFilePath = process.env.APP_CONFIG || path.join(appRoot, 'config.js'),
    checks,
    exitCodes = {
...
checks = {
    ...
    nodeVersion: function checkNodeVersion() {
        ...
        if (process.env.APP_NODE_VERSION_CHECK !== 'false' &&
            ...
            console.error('\x1B[31mApp needs Node version ' + packages.engines.node +
    ...
    packages: function checkPackages() {
        ...
        console.error('\x1B[31mERROR: The app is unable to start due to missing dependencies:\033[0m\n  ' + errors);
        console.error('\x1B[32m\nPlease run `npm install --production` and try starting the app again.');
    ...
    contentPath: function checkContentPaths() {
        ...
        var configFile,
            ...
            fd,
            errorHeader = '\x1B[31mERROR: Unable to access The app\'s content path:\033[0m',
    ...
    sqlite: function checkSqlite() {
        ...
        try {
            ...
            dbPath = path.join(appRoot, 'content', 'data', mode === 'production' ? 'app.db' : 'app-dev.db');
        }
    ...
    mail: function checkMail() {
        ...
        if (!config.mail || !config.mail.transport) {
            console.error('\x1B[31mWARNING: The app is attempting to use a direct method to send email. \nIt is recommended that you explicitly configure an email service.\033[0m');
        }
    },
    ...
    builtFilesExist: function builtFilesExist() {
        var configFile,
            config,
            location,
            fileNames = ['app.js', 'vendor.js', 'app.css', 'vendor.css'];
